// IMPORTANT: jest.mock calls are hoisted to top by Jest
// Mock upload middleware FIRST before anything else
jest.mock('../../src/middleware/upload', () => {
  ...
  // Seed directly into DB — used for GET/export/delete tests
  ...
  describe('POST /api/documents/upload', () => {
    ...
    it('uploads a txt file and returns AI analysis', async () => {
      const filePath = path.join(require('os').tmpdir(), `test-${Date.now()}.txt`);
    ...
    it('returns 400 for unsupported file type', async () => {
      const filePath = path.join(require('os').tmpdir(), `test-${Date.now()}.xyz`);
